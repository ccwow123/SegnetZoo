...
class X_Unet_v4(nn.Module):
    def __init__(self, in_ch=3, out_ch=2, base_c=32):
        ...
        self.side = SPPF(filters[4], filters[4])
        self.upx2_3 = self.upx2(filters[4], filters[3])
        self.upx2_2 = self.upx2(filters[3], filters[2])
        self.upx2_1 = self.upx2(filters[2], filters[1])
        self.upx2_0 = self.upx2(filters[1], filters[0])

    def upx2(self, in_ch, out_ch):
        net = nn.Sequential(nn.Upsample(scale_factor=2),
                      Conv(in_ch, out_ch, 1))
        return net
    def forward(self, x):
        x0 = self.Conv0(x)  # 3,256,256 -> 32,128,128
        ...
        side = self.side(x_mid)  # 512,8,8 -> 512,8,8
        ...
        side = self.upx2_3(side)  # 512,8,8 -> 256,16,16
        side = side + y_3  # 256,16,16 -> 256,16,16
        side = self.upx2_2(side)  # 256,16,16 -> 128,32,32
        side = side + y_2  # 128,32,32 -> 128,32,32
        side = self.upx2_1(side)  # 128,32,32 -> 64,64,64
        side = side + y_1  # 64,64,64 -> 64,64,64
        side = self.upx2_0(side)  # 64,64,64 -> 32,128,128
        side = side + y_0  # 32,128,128 -> 32,128,128
        out = self.segmentation_head(side)  # 32,128,128 -> 2,256,256

        return out
# v4_2直接使用UP
class X_Unet_v4_2(X_Unet_v4):
    def __init__(self,in_ch=3, out_ch=2, base_c=32):
        super().__init__(in_ch, out_ch, base_c)
    def upx2(self, in_ch, out_ch):
        net =Up(in_ch, out_ch)
        return net
    ...
    def forward(self, x):
        ...
        y_0 = self.Up0(y_1, x0)  # 64,64,64 -> 32,128,128
        # out = self.segmentation_head(y_0)  # 32,128,128 -> 2,256,256
        side = self.upx2_3(side,y_3)  # 512,8,8 -> 256,16,16
        side = self.upx2_2(side,y_2)  # 256,16,16 -> 128,32,32
        side = self.upx2_1(side,y_1)  # 128,32,32 -> 64,64,64
        side = self.upx2_0(side,y_0)  # 64,64,64 -> 32,128,128
        out = self.segmentation_head(side)  # 32,128,128 -> 2,256,256
        return out
# v5 使用maxpooling
class Down_Max(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.conv1 = nn.MaxPool2d(2, stride=2)
        self.conv2 = C3(in_ch, out_ch, n=3)
    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        return x
# v5 使用maxpooling
class X_Unet_v5(nn.Module):
    def __init__(self, in_ch=3, out_ch=2, base_c=32):
        super().__init__()
        #          0,     1,         2,        3,         4
        #           32,     64,         128,        256,         512
        filters = [base_c, base_c * 2, base_c * 4, base_c * 8, base_c * 16]
        self.Conv0 = Conv(in_ch, filters[0], 6, 2, 2)#拓展通道数
        self.Conv1 = Down_Max(filters[0], filters[1])
        self.Conv2 = Down_Max(filters[1], filters[2])
        self.Conv3 = Down_Max(filters[2], filters[3])
        self.Conv = Down_Max(filters[3], filters[4])
