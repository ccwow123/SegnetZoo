...
def get_dunet(num_class=1, backbone='resnet50', pretrained=False,
              root='~/.torch/models', pretrained_base=True, **kwargs):
    model = DUNet(num_class, backbone=backbone, pretrained_base=pretrained, **kwargs)
    return model
...
if __name__ == '__main__':
    img = torch.randn(1, 1, 560, 560)
    model = get_dunet(1, backbone='resnet101')
    with torch.no_grad():
        outputs = model(img)
    print(outputs.shape)