...
class _PSAHead(nn.Module):
    def __init__(self, nclass, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.psa = _PointwiseSpatialAttention(2048, 4900, norm_layer)
...
class _AttentionGeneration(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(reduce_x.shape,torch.softmax(attention, dim=1).shape)
...
def get_psanet(num_class=1, backbone='resnet101', pretrained=False, root='~/.torch/models',
               pretrained_base=True, **kwargs):

    model = PSANet(num_class, backbone=backbone, pretrained_base=pretrained, **kwargs)
    return model
