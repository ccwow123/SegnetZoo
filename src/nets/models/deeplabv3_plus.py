...
class DeepLabV3Plus_res(nn.Module):
    r"""DeepLabV3Plus
    Parameters
    ----------
    nclass : int
    ...
    def __init__(self, nclass, backbone='resnet50', aux=False, pretrained_base=False, dilated=False, **kwargs):
        super(DeepLabV3Plus_res, self).__init__()
        self.aux = aux
        self.nclass = nclass
        # output_stride = 8 if dilated else 32
...
class _DeepLabHead(nn.Module):
    def __init__(self, nclass, c1_channels=256, norm_layer=nn.BatchNorm2d, **kwargs):
...
def get_deeplabv3_plus(num_class=1, backbone='xception', pretrained=False, root='~/.torch/models',
    ...
    if backbone=='xception':
        model = DeepLabV3Plus(num_class, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
    else:
        model = DeepLabV3Plus_res(num_class, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
    # if pretrained:
...
if __name__ == '__main__':
    model = get_deeplabv3_plus(1, 'resnet101')
    print(model)
